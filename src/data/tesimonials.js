export const testimonials = [
  {
    id: 0,
    imageSrc: "/assets/img/testimonials/0.png",
    name: "Lucas Almeida",
    position: "Designer Web",
    comment: "Aprendizado surpreendente",
    description: `O Instituto Saber+ me proporcionou uma experiência única de aprendizado, com conteúdos inovadores e suporte eficiente.`,
  },
  {
    id: 1,
    imageSrc: "/assets/img/testimonials/1.png",
    name: "Marina Souza",
    position: "Presidente de Vendas",
    comment: "Plataforma excelente",
    description: `A plataforma é intuitiva e os cursos são muito bem estruturados. Consegui evoluir rapidamente na minha carreira.`,
  },
  {
    id: 2,
    imageSrc: "/assets/img/testimonials/2.png",
    name: "Felipe Castro",
    position: "Auxiliar de Enfermagem",
    comment: "Superou minhas expectativas",
    description: `O Instituto Saber+ superou minhas expectativas com aulas práticas e instrutores dedicados. Recomendo para todos que querem crescer.`,
  },
  {
    id: 3,
    imageSrc: "/assets/img/testimonials/3.png",
    name: "Patrícia Lima",
    position: "Designer Web",
    comment: "Contenúdo de qualidade",
    description: `Gostei muito da variedade de cursos e da facilidade de acesso ao conteúdo. O Instituto Saber+ realmente faz a diferença.`,
  },
    {
    id: 4,
    imageSrc: "/assets/img/testimonials/4.png",
    name: "Rafael Mendes",
    position: "Analista de Sistemas",
    comment: "Recomendo muito",
    description: `Os conteúdos são atualizados e os professores sempre dispostos a ajudar. Aprendi muito mais do que esperava.`,
  },
  {
    id: 5,
    imageSrc: "/assets/img/testimonials/5.png",
    name: "Juliana Torres",
    position: "Estudante de Administração",
    comment: "Experiência incrível",
    description: `A didática dos cursos é excelente e o suporte ao aluno faz toda a diferença. Me senti acolhida desde o início.`,
  },
  {
    id: 6,
    imageSrc: "/assets/img/testimonials/6.png",
    name: "Carlos Henrique",
    position: "Professor de Matemática",
    comment: "Muito satisfeito",
    description: `A plataforma oferece uma grande variedade de cursos e materiais de apoio. Consegui aprimorar minhas habilidades de forma prática.`,
  },
];

export const testimonialsTwo = [
  {
    id: 1,
    author: "Ali Tufan",
    position: "Gerente de Produto, Apple Inc.",
    text: "Não é exagero dizer que esta experiência na foi transformadora, tanto profissional quanto pessoalmente. Este workshop permanecerá por muito tempo como um ponto alto da minha vida.",
  },
  {
    id: 2,
    author: "John Smith",
    position: "Diretor de Marketing, Empresa XYZ",
    text: "O Instituto Saber+ me ajudou a adquirir habilidades valiosas que melhoraram muito minhas perspectivas de carreira. Os cursos são abrangentes e ministrados por especialistas do setor.",
  },
  {
    id: 3,
    author: "Emily Johnson",
    position: "Designer Gráfico Freelancer",
    text: "Utilizo o Instituto Saber+ há vários meses e estou impressionado com a qualidade dos cursos. Eles abrangem uma ampla gama de tópicos e são fáceis de acompanhar.",
  },
  {
    id: 4,
    author: "David Brown",
    position: "Engenheiro de Software, ABC Corporation",
    text: "Recomendo fortemente o Instituto Saber+ para quem busca aprimorar suas habilidades ou aprender novas tecnologias. A plataforma é intuitiva e os instrutores são competentes.",
  },
  {
    id: 5,
    author: "Psi. Juliana Fernandes",
    position: "Especialista em Marketing Digital",
    text: "O Instituto Saber+ mudou completamente o meu jogo. Os cursos são atualizados e relevantes, me ajudando a me manter à frente no campo em constante mudança do marketing digital.",
  },
  // Add more testimonial objects as needed
];

export const paginationImages = [
  "/assets/img/home-3/testimonials/1.png",
  "/assets/img/home-3/testimonials/2.png",
  "/assets/img/home-3/testimonials/3.png",
  "/assets/img/home-3/testimonials/4.png",
  "/assets/img/home-3/testimonials/5.png",
];

export const testimonialsTwoFour = [
  {
    id: 1,
    image: "/assets/img/home-4/testimonials/1.png",
    rating: 4.5,
    numRatings: 1991,
    text: "Acho que o Instituto Saber+ é o melhor tema que já vi este ano. Design incrível, fácil de personalizar e um design...",
    name: "Courtney Henry",
    profession: "Designer Web",
  },
  {
    id: 2,
    image: "/assets/img/home-4/testimonials/2.png",
    rating: 4.5,
    numRatings: 1991,
    text: "Acho que o Instituto Saber+ é o melhor tema que já vi este ano. Design incrível, fácil de personalizar e um design...",
    name: "Courtney Henry",
    profession: "Designer Web",
  },
  {
    id: 3,
    image: "/assets/img/home-4/testimonials/3.png",
    rating: 4.5,
    numRatings: 1991,
    text: "Acho que o Instituto Saber+ é o melhor tema que já vi este ano. Design incrível, fácil de personalizar e um design...",
    name: "Courtney Henry",
    profession: "Designer Web",
  },
];

// Adicione os textos traduzidos aqui, conforme necessário
